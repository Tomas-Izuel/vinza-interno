...
import { editarCategoriaEvento } from "@/api/categoria-evento/categoria-evento.service";
import {
...
interface EditarCategoriaEventoProps {
  categoriaEvento: CategoriaEvento;
}

export function EditarCategoriaEvento({
  categoriaEvento,
}: EditarCategoriaEventoProps) {
  const [open, setOpen] = useState(false);

  const form = useForm<EditarCategoriaEventoData>({
  ...
  const onSubmit = async (data: EditarCategoriaEventoData) => {
    try {
      await editarCategoriaEvento(categoriaEvento.id, data);
      toast.success("Ha sido editado exitosamente");
      form.reset();
      setOpen(false);
      // Refresh the page to show the updated categoria evento
      window.location.reload();
    } catch (error) {
      toast.error("Error al actualizar la categoría de evento", {
