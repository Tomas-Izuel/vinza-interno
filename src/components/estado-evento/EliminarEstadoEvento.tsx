...
import {
  canDeleteEstadoEvento,
  eliminarEstadoEvento,
} from "@/api/estado-evento/estado-evento.service";
...
interface EliminarEstadoEventoProps {
  estadoEvento: EstadoEvento;
}

export function EliminarEstadoEvento({
  estadoEvento,
}: EliminarEstadoEventoProps) {
  ...
  const handleOpenChange = async (newOpen: boolean) => {
    ...
    if (newOpen) {
      ...
      try {
        setIsChecking(true);
        const response = await canDeleteEstadoEvento(estadoEvento.id);
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      await eliminarEstadoEvento(estadoEvento.id);
    ...
    <AuthzGuard permissions={[Permissions.SUDO]}>
      <Dialog open={open} onOpenChange={handleOpenChange}>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Eliminar estado de evento</DialogTitle>
          </DialogHeader>
          ...
          {isChecking ? (
            ...
            <Alert className="mb-4">
              <AlertDescription>
                No se puede eliminar el estado del evento, tiene eventos
                relacionados.
              </AlertDescription>
            </Alert>
          ) : (
